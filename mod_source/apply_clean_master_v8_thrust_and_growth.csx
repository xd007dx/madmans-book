...
ScriptMessage("=== APPLYING CLEAN MASTER V8 (TITAN THRUST, THRUST SURGE, PUMP SURGE & PETITE TITAN OD FIX) ===");
...
// =============================================================
// 2. Step_0: Clean Logic & Titan Thrust / Petite Titan OD Fix
// =============================================================
ScriptMessage("[2/4] Patching Step_0 with 41 pills, Titan Thrust & Petite Titan OD fix...");
...
// =============================================================
// 3. Draw_0: Submenu (41 pills, 14x3 layout), 10x UI, Growth Variants & Synergies
// =============================================================
ScriptMessage("[3/4] Patching Draw_0 with 41 pills, Thrust Surge, Pump Surge & 10x rating UI...");
...
// Submenu 3 Layout (3 columns of 14 for perfect 41 pills fit)
string newSubmenuLayout = @"if (submenu == 3)
            {
                var _col = floor(i / 14);
                var _row = i % 14;
                submenu_x = room_width - 80 - (126 * _col);
                submenu_y = room_height - 24 - (28 * _row);
                _btn_w = 60;
                _spr_w = 8.2;
            }";
...
// ★ Thrust Surge (ID 39): Grow on every thrust stroke!
string oldThrustStrokeAnchor = @"if (plap == false && (orgasm == false || ds_list_find_index(pill_effects_active, UnknownEnum.Value_16) != -1))
    {
        var precum_y = 52;
        plap = true;";

string newThrustStrokeAnchor = @"if (plap == false && (orgasm == false || ds_list_find_index(pill_effects_active, UnknownEnum.Value_16) != -1))
    {
...
group.QueueFindReplace(drawCode, oldThrustStrokeAnchor, newThrustStrokeAnchor);
...
// Orgasm Pump Block: Full Container (ID 24), Sync Heart (ID 37) & Pump Surge (ID 40)
...
ScriptMessage("=== MASTER CLEAN V8 APEX PATCH APPLIED WITH 100% SUCCESS ===");
